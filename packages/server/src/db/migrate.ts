...
export async function runMigrations() {
  ...
  if (!applied) {
    await d1.exec(`
      ...
      CREATE TABLE IF NOT EXISTS topics (
        id TEXT PRIMARY KEY,
        name TEXT NOT NULL,
        kind TEXT NOT NULL CHECK(kind IN ('normal','system_cron_admin','system_artifact_pool','system_sop_library')),
        agent_type TEXT NOT NULL CHECK(agent_type IN ('programming','general')),
        pi_session_id TEXT,
      ...
      CREATE TABLE IF NOT EXISTS messages (
        ...
        role TEXT NOT NULL CHECK(role IN ('user','assistant','system','cron')),
        status TEXT NOT NULL CHECK(status IN ('streaming','done','aborted','error')),
      ...
      CREATE TABLE IF NOT EXISTS message_parts (
        ...
        kind TEXT NOT NULL CHECK(kind IN ('text','thinking','tool_use','tool_result','file_diff')),
      ...
      CREATE TABLE IF NOT EXISTS interactions (
        ...
        prompt TEXT NOT NULL,
        options_json TEXT,
        status TEXT NOT NULL CHECK(status IN ('pending','resolved','timeout')),
        response_json TEXT,
        created_at INTEGER NOT NULL,
        resolved_at INTEGER
      );
  ...
  // Run 0001: add turn_id column
  {
    ...
    if (!applied) {
      await d1.exec(`ALTER TABLE messages ADD COLUMN turn_id TEXT;`)
      await d1
        .prepare(`INSERT INTO ${MIGRATION_TABLE} (hash, created_at) VALUES (?, ?)`)
        .bind(hash, Date.now())
        .run()
  ...
  // Run 0002: add plan_mode column to topics
  {
    ...
    if (!applied) {
      await d1.exec(`ALTER TABLE topics ADD COLUMN plan_mode INTEGER NOT NULL DEFAULT 0;`)
      await d1
        .prepare(`INSERT INTO ${MIGRATION_TABLE} (hash, created_at) VALUES (?, ?)`)
        .bind(hash, Date.now())
        .run()

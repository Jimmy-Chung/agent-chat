...
export function rawTopicToTopic(raw: Record<string, unknown>): import('@agent-chat/protocol').Topic {
  return {
    ...
    sop_template_id: (raw.sop_template_id as string) ?? null,
    attention_target: (raw.attention_target as string) ?? null,
    ...
    switch (event.type) {
      ...
      case 'cron.upserted':
        useCronStore.getState().upsertCron({
          ...
          tags: (event.data as Record<string, unknown>).tags as string[] | undefined,

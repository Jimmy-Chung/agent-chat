...
- 重写 `e2e/connection-config.spec.ts`：对齐设备配对流程后的真实首屏（配对引导屏 / Agent 连接状态弹窗 / 清除配对），移除已下线的手动配置弹窗用例；e2e 共享单后端实例，playwright 改为串行（workers: 1）。
- 协议：`programmingSpecSchema` 新增 `initialPlan` / `initialTodos`，programming 话题挂载 SOP 时同样下发 plan/todos（adapter 已支持三种话题类型消费，AIT-248 配套）。
- 话题头部新增「SOP · 名称」挂载徽章，读取话题创建时固化的 sopWorkflow 快照，多个 SOP 显示 ×N 并以 tooltip 展示完整顺序。
- 版本显示更新为 `v1.10.56`。

## 2026-06-11 [v1.10.54] — feat: 应用内产物在线预览弹窗

- 产物资源点击后优先在当前页面弹窗预览，支持 PDF、图片、Markdown、HTML 与常见文本文件。
- 无法在线展示、文件过大或预览加载失败时，保留原有新标签页浏览器打开行为。
- 抽出共享产物访问按钮供话题面板和检查器面板复用，下载行为保持不变。
- 补充 PDF.js、Viewer.js、markdown-it 与 iframe sandbox 相关单测覆盖。
- 版本显示更新为 `v1.10.54`。

## 2026-06-11 [v1.10.53] — fix: 历史消息引用 CI 回归

- 修复历史消息引用投递逻辑在 server delivery 单测 mock 未返回 parts 数组时抛错的问题，避免发送流程误判为 `retryable`。
- 版本显示更新为 `v1.10.53`。

## 2026-06-11 [v1.10.52] — feat: 历史消息引用

- 新增当前会话历史消息整条引用：消息 hover 可加入输入框引用区，发送时保留引用快照。
- assistant 请求会显式携带被引用历史消息上下文，避免长对话裁剪后只靠上下文猜测。
- 消息列表展示引用来源并支持点击跳回原消息；排队消息也会保留引用关系。
- 版本显示更新为 `v1.10.52`。
...
- 修复 artifact 签名 token 解码中文 key 时没有按 UTF-8 还原的问题，避免中文文件名参与上传/下载校验时被解坏。
- 版本显示更新为 `v1.10.50`。

## 2026-06-10 [v1.10.49] — fix: 输入框交互优化与简洁回答开关

- 修复状态横条层叠越级遮挡侧边栏：给主面板 `<main>` 加 `isolate` 隔离层叠上下文，横条 `z-30` 不再逃逸到外层盖住侧边栏。
- 状态横条的连接状态闪烁圆点外圈从 24px 缩到 14px，更贴合圆点。
- 移除输入框右下角的 Stop 按钮，恒显发送按钮；中断回复改用状态栏 Stop，回复中再发走发送队列。
- 输入框 Model 选择器右侧新增「DOUBAO MODE」开关：开启时在该条消息末尾拼接简洁作答指令，按话题独立、发送后自动关闭。
- 版本显示更新为 `v1.10.49`。

## 2026-06-10 [v1.10.48] — fix: 产物池引用删除与上传预览

- 删除话题弹窗默认改为删除产物引用，文案明确不会删除 adapter 本机文件；保留引用到产物池仍作为显式选项。
- 产物池新增名称搜索、多选和批量删除确认；删除只移除 agent-chat 引用与 R2 云端数据，并拦截仍被活跃话题引用的产物。
- 产物池路径在 workspace root 下显示为 `${workspace}/...`，系统话题不再显示右侧 Attention/Todo Inspector 面板。
- 修复 adapter 后续补发 `r2Key` 时 artifact 记录未更新导致 `.md` 预览/下载提示不可用的问题，并串行化前端上传队列。
- 版本显示更新为 `v1.10.48`。

## 2026-06-10 [v1.10.47] — fix: Attention mirror 全尺寸面板标题行

- Inspector 展开的 Attention 全尺寸面板补上 S16 设计稿的 52px 标题行：Attention 图标 + 标题 + 分析中提示，「导出 SOP」与「收起」按钮从画布右上角悬浮位收进标题行。
- 与话题入口 `AttentionDrawer` 的标题行视觉对齐（毛玻璃底 + hairline 分隔线），两个入口样式统一。
- 版本显示更新为 `v1.10.47`。
...
- 左侧图节点在导出模式下可直接勾选，并支持在导出弹窗内展开/收起聚合节点；原注意力面板不传入导出选择 props，避免引入选择状态副作用。
- 移除上一版列表缩进选择器逻辑和对应测试，保留源节点按 trace 顺序去重的导出回归测试。
- 版本显示更新为 `v1.10.44`。

## 2026-06-10 [v1.10.43] — fix: SOP 导出入口可用性与多级缩进

- `导出 SOP` 外层入口不再直接按 `nodes.length` 禁用，避免旧话题/加载态下明明有 projection 节点但入口不可点；是否可提交仍由弹窗内的名称、选择和预览结果控制。
- SOP 导出选择器的缩进改为根据 projection 的展开边关系计算，支持多级展开层级；普通主线边不再影响缩进层级。
- 新增测试覆盖展开边多级缩进和普通主线边不缩进。
- 版本显示更新为 `v1.10.43`。

## 2026-06-10 [v1.10.42] — fix: SOP 导出节点选择器层级缩进

- 调整 `导出 SOP` 选择弹窗的节点列表样式：展开聚合节点后，子层节点按 depth 缩进显示，并在左侧增加细层级线，避免父聚合节点与展开子节点视觉层级混淆。
- 顶层节点保持原位置，选择、展开和导出逻辑不变。
- 版本显示更新为 `v1.10.42`。

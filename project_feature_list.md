...
| 类别 | 需求 | 状态 | 说明 |
|---|---|---|---|
| 创建话题 | AIT-177 工作区根目录快捷选择 | 已回归 | 新建 Programming 话题时，Working Directory 输入 `/` 可浏览 Adapter workspace 一级目录；输入 `/项目名` 解析为 workspace 下的绝对路径，无匹配时作为新工程目录使用 |
...
| 类别 | 需求 | 状态 | 说明 |
|---|---|---|---|
| 定时任务 | AIT-185 agent-chat 侧 cron 投影改造 | 已回归 | adapter 作为 cron 定义与 run history 真源；agent-chat server 仅保留兼容投影，对外统一使用 adapter `cronId`，D1 行 ID 仅作为 `localCronId` |
...
| 类别 | 需求 | 状态 | 说明 |
|---|---|---|---|
| 定时任务 | AIT-186 cron 标签贯通与搜索 | 已回归 | cron 支持 `tags` 贯通协议/投影/UI，cron admin 可按标签、表达式、任务名、来源话题搜索 |
...
| 类别 | 需求 | 状态 | 说明 |
|---|---|---|---|
| 趣味功能 | FEAT-037 凌晨睡觉提醒弹窗 | 已发布 | GMT+8 凌晨 0 点起每 5 分钟弹窗，指数级点击关闭，1 点后自动禁用 |

> 范围说明：本版本仅包含趣味性睡觉提醒功能。

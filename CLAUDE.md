...
- 用户报告 bug → 追加至 project_bug_list.md（状态：新建），ID 递增（BUG-XXX）
- Bug 状态机：新建 → 处理中 → 已修复 → 重新打开
- 建单后确认不是 bug 或无需修复 → 状态置为「已关闭」
- 修复后 → 更新状态为「已修复」，填写根因与修复方案
- 新会话启动时主动提醒是否有 `新建`、`处理中` 或 `重新打开` 的 bug 条目
...
5. 修复后跑回归；通过 → 状态「已修复」；不通过或用户复现 → 状态「重新打开」并回到第 1 步
...
2. 确认 → 归属版本；需求状态机固定为「待讨论 → 开发中 → 测试中 → 待发布 → 已发布」
3. 待讨论阶段 → 写清需求、验收标准、TC 场景清单；此时 TC 是产品/行为级测试点，不要求已有自动化代码
4. 开发阶段 → 根据 TC 编写对应单元测试、组件测试或集成测试，再实现代码
5. 完成标准 → 相关自动化测试通过后，需求进入「待发布」；发布完成后进入「已发布」

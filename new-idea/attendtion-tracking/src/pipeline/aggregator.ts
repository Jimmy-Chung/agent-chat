import { RawEvent, TraceNode, PlanItem, AssistantActionKind, UserMessageKind, TraceExchange } from '../types'
...
export function aggregate(events: RawEvent[]): {
  ...
  for (const evt of events) {
    if (evt.kind === 'plan' || evt.kind === 'message') {
      const items = evt.payload.items as Array<{
        id: string; text: string; status: string; depth: number
      }> | undefined
      if (items?.length) {
  ...
  // ── 以用户消息为边界，分组所有事件 ─────────────────────────────────────────
  // 找到所有真实的用户消息（kind=message, role=user，且不是系统注入的 XML 通知）
  ...
  if (userMsgEvents.length === 0) {
    // 没有用户消息（纯工具调用日志）→ 退化为单节点
    return { candidates: fallbackSingleCandidate(events), planItems: compactPlanItems(planItems) }
  }

  // 按顺序遍历，遇到真实用户消息就开启新组
  // 不依赖时间戳（Claude Code JSONL 里多个事件可能时间戳相同）
  ...
  const flushGroup = () => {
    ...
    const exchange = buildExchange(currentUserEvt, currentThinking, currentTools, currentMessages, lastAiSummary)
...
// 判断是否是真实的人类输入（过滤 Claude Code 系统注入的 XML 通知）
function isHumanMessage(text: string | undefined): boolean {
  if (!text?.trim()) return false
  const t = text.trim()
  // 系统注入的消息以 XML 标签开头，如 <task-notification>、<search_results> 等
  if (t.startsWith('<')) return false
  // 过滤掉纯 JSON 内容（也可能是系统消息）
...
function buildExchange(
  ...
  const allTs = [userEvt.ts, ...thinking.map((e) => e.ts), ...tools.map((e) => e.ts), ...messages.map((e) => e.ts)]
  return {
    id: `ex_${userEvt.id}`,
...
// 把一个 Phase 的 TraceExchange[] 聚合成 ≤ maxGroups 个 ExchangeGroup
// 每个 ExchangeGroup 可再次钻入，形成递归层级

export type ExchangeGroup = {
  id: string
  exchanges: TraceExchange[]     // 该组包含的原始 exchanges
  user_message: string           // 代表性用户消息（第一条 question/instruction 优先）
  user_kind: UserMessageKind
  assistant_summary: string      // 代表性 AI 回复（最后一条）
  tool_count: number
  ts_start: number
  ts_end: number
...
function mergeUserKindForGroup(kinds: UserMessageKind[]): UserMessageKind {
  const rank: Record<UserMessageKind, number> = {
    question: 5, proposal: 4, choice: 3, instruction: 2, evidence: 1,
  }
...
export function groupExchanges(exchanges: TraceExchange[], maxGroups = 12): ExchangeGroup[] {
  ...
  // ── 1. 语义分组（按 question/proposal 作为新组边界）────────────────
  ...
  return rawGroups.map((group, i) => {
    // 选最高优先级的 user_message 作为代表
    const representative = [...group].sort((a, b) => {
      const rank: Record<UserMessageKind, number> = {
        question: 5, proposal: 4, instruction: 3, choice: 2, evidence: 1,
      }
